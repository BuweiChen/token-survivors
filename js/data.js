...
const DATA = (() => {
  ...
  const WEAPONS = {
    tokenStream: {
      ...
      base: { dmg: 8, cd: 1.05, count: 1, speed: 430, pierce: 0 },
      lvls: [{ count: 1 }, { dmg: 4 }, { count: 1 }, { cd: -0.15 }, { dmg: 5 }, { count: 1 }, { pierce: 1 }, { dmg: 6, cd: -0.1 }],
    ...
    attention: {
      ...
      lvls: [{ count: 1 }, { dmg: 8 }, { count: 1 }, { range: 120 }, { dmg: 10 }, { count: 1 }, { cd: -0.4 }, { dmg: 14, count: 1 }],
    ...
    contextWindow: {
      ...
      base: { dmg: 5, cd: 0.5, radius: 85 },
      lvls: [{ radius: 14 }, { dmg: 2 }, { radius: 14 }, { dmg: 2 }, { radius: 16 }, { dmg: 3 }, { radius: 18 }, { dmg: 4, radius: 14 }],
      evolvesWith: 'vram',
      evo: 'fable5',
    },
    chainOfThought: {
      ...
      base: { dmg: 6, life: 2.4, radius: 26, drop: 0.10 },
      lvls: [{ life: 0.6 }, { dmg: 3 }, { radius: 6 }, { life: 0.6 }, { dmg: 3 }, { radius: 6 }, { life: 0.8 }, { dmg: 5, radius: 8 }],
      evolvesWith: 'kvCache',
      evo: 'deepseekR1',
    },
    rag: {
      ...
      base: { dmg: 12, cd: 1.7, count: 1, speed: 380, range: 0.55 },
      lvls: [{ count: 1 }, { dmg: 5 }, { range: 0.12 }, { count: 1 }, { dmg: 6 }, { range: 0.12 }, { count: 1 }, { dmg: 8 }],
    ...
    embeddings: {
      ...
      base: { dmg: 10, count: 2, radius: 72, rotSpeed: 2.6 },
      lvls: [{ count: 1 }, { dmg: 5 }, { rotSpeed: 0.7 }, { count: 1 }, { dmg: 5 }, { radius: 18 }, { count: 1 }, { dmg: 7, rotSpeed: 0.7 }],
    ...
    temperature: {
      ...
      base: { dmg: 4, cd: 0.09, spread: 0.25, range: 230, burst: 1 },
      lvls: [{ spread: 0.08 }, { dmg: 2 }, { range: 35 }, { spread: 0.08, burst: 1 }, { dmg: 2 }, { range: 35 }, { cd: -0.02 }, { dmg: 3, burst: 1 }],
    ...
    gradientDescent: {
      ...
      base: { dmg: 24, cd: 1.7, count: 2, radius: 52 },
      lvls: [{ count: 1 }, { dmg: 8 }, { radius: 10 }, { count: 1 }, { dmg: 10 }, { radius: 12 }, { cd: -0.35 }, { dmg: 12, count: 1 }],
      evolvesWith: 'lora',
      evo: 'gemini',
    },
    toolCall: {
      ...
      base: { dmg: 10, cd: 4.5, count: 1, life: 8, fireCd: 0.45, range: 330 },
      lvls: [{ dmg: 4 }, { fireCd: -0.07 }, { count: 1 }, { life: 3 }, { dmg: 5 }, { count: 1 }, { fireCd: -0.08 }, { dmg: 7, life: 3 }],
    ...
    hallucination: {
      ...
      base: { dmg: 16, cd: 1.25, count: 3, speed: 300, critCh: 0.22 },
      lvls: [{ count: 1 }, { dmg: 6 }, { count: 1 }, { critCh: 0.08 }, { dmg: 7 }, { count: 1 }, { critCh: 0.08 }, { dmg: 9, count: 1 }],
      evolvesWith: 'rlhf',
  ...
  const META = {
    params:  { name: 'Moar Parameters', icon: '\uD83E\uDDEE', max: 5, cost: l => 60 * (l + 1), desc: '+8% damage / rank', stat: 'might', per: 0.08 },
    h100s:   { name: 'Rack of H100s', icon: '\uD83D\uDDA5\uFE0F', max: 5, cost: l => 50 * (l + 1), desc: '+12 max HP / rank', stat: 'maxhp', per: 12 },
    epochs:  { name: 'More Epochs', icon: '\uD83D\uDD01', max: 5, cost: l => 50 * (l + 1), desc: '+6% XP / rank', stat: 'growth', per: 0.06 },
    sgd:     { name: 'Momentum (SGD)', icon: '\uD83D\uDEFC', max: 3, cost: l => 70 * (l + 1), desc: '+5% move speed / rank', stat: 'speed', per: 0.05 },
    cherry:  { name: 'Cherry-picked Evals', icon: '\uD83C\uDF52', max: 3, cost: l => 80 * (l + 1), desc: '+8% luck / rank', stat: 'luck', per: 0.08 },
